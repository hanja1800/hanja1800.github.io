 ...
 * Reverted to Simple Data Loading (data.json)
 */
...
class HanjaApp {
    ...
    init() {
        this.loadData();
        this.bindEvents();
        console.log('📚 HanjaApp Initialized (Legacy Mode)');
    }

    // ==========================================
    // Data Loading & Processing (단일 data.json 버전으로 복구)
    // ==========================================
    async loadData() {
        if (this.dom.loadingMsg) this.dom.loadingMsg.style.display = 'block';

        try {
            // 다시 예전처럼 data.json 파일 하나만 직접 불러옵니다.
            ...
            this.updateUI();
            console.log(`✅ Loaded ${this.state.data.length} Hanja entries from data.json`);
