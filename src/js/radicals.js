// ==================== Global Variables ====================
import { DarkModeManager, FavoritesManager, RecentHistoryManager, renderHanjaRow } from './common.js';
...
function displayStrokes() {
    ...
    let html = `
        <div class="stroke-buttons" id="strokeButtonsContainer">
            <button class="stroke-btn active" data-stroke="0">전체</button>
    `;

    strokes.forEach(stroke => {
        html += `
            <button class="stroke-btn" data-stroke="${stroke}">
                ${stroke}획
            </button>
        `;
    });
    ...
    // Add event delegation for stroke buttons
    const btnContainer = document.getElementById('strokeButtonsContainer');
    if (btnContainer) {
        btnContainer.addEventListener('click', (e) => {
            const btn = e.target.closest('.stroke-btn');
            if (btn) {
                const stroke = parseInt(btn.dataset.stroke, 10);
                selectStroke(stroke, btn);
            }
        });
    }
