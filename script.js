...
function addToRecent(item) {
    // 필요한 정보만 객체로 저장
    const historyItem = {
        hanja: getField(item, '한자'),
        huneum: getField(item, '훈음'),
        gubun: getField(item, '구분'),
        url: getField(item, 'URL'),
        grade: getField(item, '급수'),
        timestamp: Date.now()
    };
    
    // 중복 제거 (훈음이 같으면 삭제 후 다시 추가)
    recentHistory = recentHistory.filter(h => h.huneum !== historyItem.huneum);
...
function renderRecentList() {
    ...
    recentHistory.forEach((item, index) => {
        ...
        li.innerHTML = `
            ...
            <!-- ▼▼▼ 여기 수정됨: deleteRecentItem(${index}, event) ▼▼▼ -->
...
window.deleteRecentItem = function(index, event) {
    // ▼▼▼ 핵심: 클릭 이벤트가 document까지 올라가서 모달을 닫는 것을 방지 ▼▼▼
    if (event) {
...
function adjustTableHeaderOffset() {
    ...
    // 검색 영역의 현재 높이 측정 (여유분 1~2px 제외 가능)
...
// 3. 필터 칩이 추가/삭제되어 높이가 변할 때를 감지 (MutationObserver 사용)
...
// 4. 스크롤 시에도 미세 조정 (모바일 주소창 등으로 인한 높이 변화 대응)
