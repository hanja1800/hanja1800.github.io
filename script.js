...
function loadRecentlyViewed() {
    const saved = localStorage.getItem('hanja-recently-viewed');
    if (saved) {
        recentlyViewed = JSON.parse(saved).slice(0, 30);
        updateRecentBadge();
    }
}

function saveRecentlyViewed() {
    localStorage.setItem('hanja-recently-viewed', JSON.stringify(recentlyViewed));
}

function addToRecentlyViewed(item) {
    const key = `${getField(item, '훈음')}|${getField(item, '구분') || ''}`;
    recentlyViewed = recentlyViewed.filter(x => x.key !== key);
    recentlyViewed.unshift({ ...item, key });
    if (recentlyViewed.length > 30) recentlyViewed.pop();
    saveRecentlyViewed();
    updateRecentBadge();
}
...
function updateRecentBadge() {
    ...
    if (badge) badge.textContent = recentlyViewed.length || '0';
}
...
function renderRecentModal() {
    ...
    list.innerHTML = recentlyViewed.map(item => {
        const hanja = getField(item, '한자') || getField(item, '훈음');
        const isEnd = (getField(item, '구분') || '').includes('끝음절');
        const num = isEnd ? '²' : '¹';
        const display = isEnd ? `-${hanja}<sup>${num}</sup>` : `${hanja}<sup>${num}</sup>-`;
        
        return `<div class="recent-modal-item" data-key="${item.key}">
            ${display}
        </div>`;
    }).join('');
}
...
// 모달 열기/닫기
document.getElementById('openRecentModal')?.addEventListener('click', () => {
...
document.getElementById('recentModal')?.addEventListener('click', e => {
...
// 항목 클릭 → 블로그 즉시 열기
document.getElementById('recentModalList')?.addEventListener('click', e => {
    const item = e.target.closest('.recent-modal-item');
    if (!item) return;
    const key = item.dataset.key;
    const found = recentlyViewed.find(x => x.key === key);
...
document.getElementById('tableBody').addEventListener('click', e => {
    ...
    const huneum = row.cells[1].textContent.trim();
    const gubun = row.cells[2].textContent.trim();

    const foundItem = hanjaData.find(item => 
        getField(item, '훈음') === huneum && 
        (getField(item, '구분') || '').trim() === gubun
    );

    if (foundItem && getField(foundItem, 'URL')) {
        // 즉시 열기
        window.open(getField(foundItem, 'URL'), '_blank');
        // 백그라운드 저장 (딜레이 0)
        setTimeout(() => addToRecentlyViewed(foundItem), 0);
    }
});
