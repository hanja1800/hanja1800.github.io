...
document.getElementById('tableBody').addEventListener('click', e => {
    ...
    const currentDataItem = [...document.querySelectorAll('#tableBody tr')].indexOf(row);
    const actualIndex = (currentPage - 1) * itemsPerPage + currentDataItem;
    const item = sortedHanjaData.filter(filterData().includes.bind(filterData()))[actualIndex];
    if (item) addToRecentlyViewed(item);
});
