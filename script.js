...
updateGradeButtonLabel();
// ===== 최근 본 한자 기능 (localStorage) =====
let recentlyViewed = [];

function loadRecentlyViewed() {
    const saved = localStorage.getItem('hanja-recently-viewed');
    if (saved) {
        recentlyViewed = JSON.parse(saved).slice(0, 30); // 최대 30개
...
function updateRecentlyViewedUI() {
    ...
    container.classList.add('show');
    count.textContent = `(${recentlyViewed.length})`;

    list.innerHTML = recentlyViewed.map(item => {
        const gradeClass = getGradeClass(getField(item, '급수'));
        return `
            <div class="recent-item" data-huneum="${getField(item, '훈음')}" data-gubun="${getField(item, '구분') || ''}">
                <div class="hanja">${getField(item, '훈음')}</div>
                <div class="info">
                    ${getField(item, '교육수준') || '—'} · 
                    <span class="grade-badge ${gradeClass}" style="font-size:0.7rem;padding:2px 6px;">
                        ${getField(item, '급수') || '—'}
                    </span>
                </div>
            </div>
...
document.getElementById('tableBody').addEventListener('click', e => {
    ...
    if (foundItem) {
        const url = getField(foundItem, 'URL');
        if (url) {
            // 1. 최근 본 한자에 저장
            addToRecentlyViewed(foundItem);
            // 2. 블로그 새 탭으로 열기
