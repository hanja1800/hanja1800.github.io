        ...
        AppState.recentHistory.forEach((item, index) => {
            ...
            li.innerHTML = `
                <a href="${escapeHtml(item.url)}" target="_blank" class="recent-item-link" 
                   title="새 탭에서 보기" rel="noopener noreferrer">
                ...
                <button class="delete-recent-btn" data-index="${index}" 
                        aria-label="삭제" title="기록에서 삭제">×</button>
            `;
...
const Filter = {
    ...
    applyAndRender() {
        const searchTerm = AppState.elements.searchInput.value.toLowerCase();
        const education = AppState.elements.educationFilter.value;
        const length = AppState.elements.lengthFilter.value;
        
        const filtered = AppState.sortedHanjaData.filter(item => {
            const hanja = item['한자'] || '';
            const eum = item['음'] || '';
            ...
            const matchGrade = AppState.selectedGrades.length === 0 || 
                AppState.selectedGrades.includes(geubsu);
            const matchLength = !length || jangdaneum === length;
            const matchFavorites = !AppState.showOnlyFavorites || 
                Favorites.isFavorite(huneum, gubun);
            
            let matchChosung = true;
            if (AppState.selectedSyllable) {
                matchChosung = eum === AppState.selectedSyllable;
            } else if (AppState.selectedChosung) {
                const normalized = HangulUtils.normalizeChosung(
                    HangulUtils.getChosung(eum.charAt(0))
                );
                matchChosung = normalized === AppState.selectedChosung;
            }
            
            const notEnding = !AppState.selectedSyllable || !gubun.includes('끝음절');
...
async function loadData() {
    try {
        ...
        console.log(`✅ ${AppState.hanjaData.length}개의 한자 데이터 로드 완료`);
        return true;
    } catch (error) {
        ...
        if (AppState.elements.tableBody) {
            AppState.elements.tableBody.innerHTML = 
                `<tr><td colspan="7" style="text-align:center;padding:40px;">
                    데이터 로드 실패<br>${escapeHtml(error.message)}
                </td></tr>`;
        }
        return false;
    }
